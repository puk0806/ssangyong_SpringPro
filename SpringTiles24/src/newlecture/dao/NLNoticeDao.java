 ...
 */
	...
	*/
	...
	public void hitUp(String seq) {
		...
		String threadName = Thread.currentThread().getName();	// 현재 작동중인 쓰레드 이름 얻어오기
		System.out.println("쓰레드 : "+threadName+"  조회수 증가 전");
		
		this.jdbcTemplate.update(sql, paramSource);
		
		System.out.println("쓰레드 : "+threadName+"  조회수 증가 후");
		threadSleep(threadName,1000);
		
		System.out.printf("%s 조회수 증가 롤백\n","hipUp()");
		throw new RuntimeException();	// 트랜잭션 롤백처리
	}
	*/
	...
	// 쓰레드 재우는 함수
	private void threadSleep(String threadName, long duration) {
		System.out.printf("%s Sleep 시작\n",threadName);
		...
		System.out.printf("%s Sleep 종료\n",threadName);
	}


	// 조회스 얻어오는 메소드
	@Override
	// @Transactional(isolation = Isolation.READ_UNCOMMITTED)		// 오라클에서 지원하지 않음
	...
	public int getHit(String seq) {
		...
		Map<String, Object> parampMap = new HashMap<String, Object>();
		parampMap.put("seq", seq);
		
		int hit = this.jdbcTemplate.queryForInt(sql, parampMap);
		System.out.println("getHit() :"+hit);
